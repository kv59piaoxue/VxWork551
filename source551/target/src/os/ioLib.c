...
--------------------
04z,31jan03,pmr  SPR 82273: fixed linkCount declaration in ioCreateOrOpen().
		 SPR 80447: fixed remove() to follow symbolic links.
04y,13jan03,gls  moved check for NULL name in ioCreateOrOpen() (SPR #82909)
...
handleError:
    ...
    if (error > 2)
	{
	iosDelete (pDevHdr1, fullFileName);
	}
    if (error > 1)
	{
	iosFdFree (fd);
...
STATUS remove
    ...
    value = iosDelete (pDevHdr, fullFileName);
