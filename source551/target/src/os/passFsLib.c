...
--------------------
01s,19dec02,jmp  Implemented statfs and fstatfs for passFs (SPR 29024).
...
/*******************************************************************************
*
* passFsFileStatfsGet - get file system status (directory entry data)
*
* This routine is called via an ioctl() call, using the FIOFSTATFSGET
* function code.  The passed statfs structure is filled, using data
* obtained from the directory entry which describes the file.
*
* RETURNS: ERROR or OK
*/

LOCAL STATUS passFsFileStatfsGet
    (
    PASS_FILE_DESC      *pPassFd,       /* pointer to file descriptor */
    struct statfs       *pStatfs        /* structure to fill with data */
    )

    {
    struct unix_statvfs unixbuf;

    if (u_statvfs (pPassFd->passName, (char*)&unixbuf) == -1)
        return (ERROR);

    bzero ((char *) pStatfs, sizeof (struct statfs));/* zero out statfs struc */

    /* Fill statfs structure */

    pStatfs->f_bsize    = unixbuf.f_frsize;      /* file system block size */
    pStatfs->f_blocks   = unixbuf.f_blocks;      /* total number of blocks */
    pStatfs->f_bfree    = unixbuf.f_bfree;       /* number of free blocks */
    pStatfs->f_bavail   = unixbuf.f_bavail;
                                        /* free blocks avail to non-superuser */
    pStatfs->f_files    = unixbuf.f_files;       /* number of file nodes */
    pStatfs->f_ffree    = unixbuf.f_ffree;       /* number of free file nodes */
    pStatfs->f_fsid.val[0]= unixbuf.f_fsid;      /* file system id */
...
#if CPU == SIMSPARCSOLARIS
	case FIOFSTATFSGET:
            retValue = passFsFileStatfsGet (pPassFd, (struct statfs *) arg);
            break;
#endif /* CPU == SIMSPARCSOLARIS */
