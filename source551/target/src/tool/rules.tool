...
# --------------------
# 01o,03jun03,sn   cleaned up GCC 3.x support
# 01n,07apr03,dtr  Adding gnu version specific rules.
...
PRE_OBJS := $(patsubst %,$(OBJ_PREFIX)%,$(REAL_OBJS)) $(EXTRA_OBJS)
...
# to worry about whether or not the compiler prepends an underscore.
	echo char __$(subst +,_,$(subst -,_,$(notdir $*)))_o = 0$(SEMI) > $(WRAPPER)
ifeq ($(WIND_HOST_TYPE),x86-win32)
