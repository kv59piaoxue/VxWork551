...
01j,13jan03,gls  made pthread_cond_wait() do atomic release/wait (SPR #85235)
		 ...
		 added check for waiting thread in pthread_cond_signal (SPR #85264)
...
#define INIT_MUTEX(pMutex)						\
	...
	if ((pMutex)->mutexInitted == FALSE)				\
		...
		if ((pMutex)->mutexSemId == NULL)			\
		    {							\
		    *((volatile int *)&pMutex->mutexIniting) = FALSE;   \
		    taskUnlock ();					\
		    return (EINVAL);	/* internal error */		\
		    }							\
									\
		*((volatile int *)&(pMutex)->mutexInitted) = TRUE;	\
					   				\
		taskUnlock();	/* may not be necessary */		\
		}							\
	    else							\
		{							\
		while (*((volatile int *)&(pMutex)->mutexInitted) == FALSE) \
		    {							\
		    if (*((volatile int *)&pMutex->mutexIniting) == FALSE) \
			{						\
			/* semMCreate we were waiting for failed */	\
					   				\
			return (EINVAL);				\
			}						\
...
#define INIT_COND(pCond)						\
	...
	if ((pCond)->condInitted == FALSE)				\
		...
		if ((pCond)->condSemId == NULL)				\
		    {							\
		    *((volatile int *)&pCond->condIniting) = FALSE;	\
		    taskUnlock ();					\
		    return (EINVAL); /* internal error */		\
		    }							\
									\
		*((volatile int *)&(pCond)->condInitted) = TRUE;	\
									\
		taskUnlock(); /* may not be necessary */		\
		}							\
	    else							\
		{							\
		while (*((volatile int *)&(pCond)->condInitted) == FALSE) \
		    {							\
		    if (*((volatile int *)&pCond->condIniting) == FALSE)  \
			{						\
			/* semBCreate we were waiting for failed */	\
					   				\
			return (EINVAL);				\
			}						\
		    /* wait as short a time as possible */		\
