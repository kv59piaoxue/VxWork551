...
--------------------
05r,09jun03,dtr  CP1 merge.
05q,22apr03,dtr  Changing slot name from reserved2 to compiler1.
05p,10jan03,pcm  added VX_DEALLOC_STACK option to taskInit() documentation;
		 initialized task name to NULL in taskInit();
		 made sure all WIND_TCB elements initialized in taskInit();
		 (SPRs 30602, 84273, 77118 respectively)
05p,04sep02,dtr  Adding support for SPE.
...
WIND_TCB *      pTaskLastSpeTcb     = NULL; /* pTcb for speSwapHook */
...
*	VX_ALTIVEC_TASK		-  execute with Altivec support
*	VX_SPE_TASK		-  execute with SPE support
...
* VX_DEALLOC_STACK bit in <options>; taskSpawn() always sets this bit.  Setting
* this bit causes the stack to be automatically deallocated when a taskDelete()
* is done, or when a return command is issued from the entry function.  If a
* pre-allocated stack is used, the VX_DEALLOC_STACK bit must not be set.
*
...
STATUS taskInit
    ...
    pTcb->name		= NULL;			/* init name ptr to NULL */
    ...
    bzero ((char *)&(pTcb->wdbInfo), sizeof (WDB_INFO));
...
#ifdef _WRS_SPE_SUPPORT
    /*
     * If the task being deleted is the last Spe task from 
     * speSwapHook then reset pTaskLastSpeTcb.
     */

    if (pTcb == pTaskLastSpeTcb)
        pTaskLastSpeTcb = NULL;
#endif /* _WRS_SPE_SUPPORT */
