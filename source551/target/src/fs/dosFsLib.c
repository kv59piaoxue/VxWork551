...
02u,31jan03,jkf  SPR#85305, flush cache after chkdsk repair, 
                 ...
                 SPR#76352, sync fat copy on dir create in dosFsCloseon dir create in dosFsClose
...
STATUS dosFsChkDsk
    ...
    /* flush and invalidate CBIO dcaches */
...
LOCAL STATUS dosFsSeek
    ...
    if( newPos < pFd->pos || pFd->curSec == 0 )	/* backward */
    	...
    	nSec = NSECTORS( pFd->pVolDesc,
                        min( newPos, pFd->pFileHdl->size - (fsize_t) 1 ) );
         ...
         */
        nSec = NSECTORS( pFd->pVolDesc,
                        (min( newPos, pFd->pFileHdl->size - 1 ) - pFd->pos));
...
LOCAL STATUS dosFsFillGap
    ...
    FAST CBIO_DEV_ID	pCbio = pFd->pVolDesc->pCbio; /* cbio handle */
     ...
     * boundary, then we will not need to fill any sectors.  In this
     * case the sector has already been zeroed when it was originally
     * allocated. We just need to update pFd->pos and pFileHdl->size.
     */

    if( ((fsize_t)( seekOutPos - pFd->pos )) <
         (fsize_t)( (fsize_t)(pVolDesc->bytesPerSec) - 
                 (fsize_t)(OFFSET_IN_SEC( pVolDesc, pFd->pos))))
        {
    	pFileHdl->size = seekOutPos;
        pFd->pos = seekOutPos;
        return (OK);
        }

    /* 
     * If this is a non-zero length file, and we are not positioned 
     * upon a sector boundary, then skip over anything remaining 
     * in the current sector since it was already zero'd.
     */

    if ((0 < pFileHdl->size) && (0 != OFFSET_IN_SEC( pVolDesc, pFd->pos )))
        {
        /* 
         * Thou shalt only decrement pFd->nSec if it is non-zero.
         * A zero pFd->nSec indicates that no cluster has been 
         * allocated yet via pFatDesc->getNext() function.  This 
         * protects against the error condition of pFd->nSec being
         * decremented from zero and ensures a call to pFatDesc->getNext()
         * below.
         */
        ...
        if (0 != pFd->nSec)
    	    ...
    	    pFd->nSec --;
    	    pFd->curSec ++;
    	    pFd->pos += (fsize_t)(min (seekOutPos - pFd->pos,
    ...
    while( pFd->pos < seekOutPos )
    	...
    	pFd->pos += (fsize_t) 
                    (min( seekOutPos - pFd->pos, pVolDesc->bytesPerSec));
    	pFileHdl->size = pFd->pos;
