...
--------------------
01p,31jan03,jkf   SPR#84632 (21nov02,jch - dosChkLostFree now forces 
                  recalculation of cached free cluster count during repairs)
...
LOCAL STATUS dosChkLostFree
    ...
    DOS_VOLUME_DESC_ID 	pVolDesc = pFd->pVolDesc;
    DOS_FAT_DESC_ID	pFatDesc = pVolDesc->pFatDesc;
    MS_FAT_DESC_ID	pMSFatDesc = (void *) pVolDesc->pFatDesc;
    uint32_t		entNum;
    uint32_t		nextEnt;
    uint32_t		nextEntCont;
    uint32_t		entVal;
	...
	/* Force recalculation of cached free cluster count */
	
	pMSFatDesc->fatEntFreeCnt = -1;
	pVolDesc->pFatDesc->nFree(pFd);

	/* If FAT32, save new value to the File System Information Sector */
	
	if (pVolDesc->fatType == FAT32) {
            VX_TO_DISK_32 (pMSFatDesc->fatEntFreeCnt, &fsinfoBuf[0]);
            VX_TO_DISK_32 (pMSFatDesc->groupAllocStart, &fsinfoBuf[4]);
            cbioBytesRW (pFd->pVolDesc->pCbio, FSINFO_SEC_NUM, FSINFO_FREE_CLUSTS, 
		         (addr_t) fsinfoBuf, sizeof(fsinfoBuf), CBIO_WRITE, NULL);
	    cbioIoctl (pFd->pVolDesc->pCbio, CBIO_CACHE_FLUSH, (void *) (-1));
        }
