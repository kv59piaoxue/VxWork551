...
--------------------
01p,14jan03,rae  Merged from velocecp branch (SPR 83470)
	...
	register int len;
{
	...
	fd_set mask;					/* 4.0 */
	fd_set readFds;
	FAST struct svc *ms = &taskRpcStatics->svc;
 
	do {
		int bytes = howmany(FD_SETSIZE, NFDBITS) * sizeof (fd_mask);

		/*
		 * Monitor all of the RPC service sockets rather than
		 * just the one for this client. This avoids a denial
		 * of service condition where one client can prevent
		 * any other clients from using a given TCP service
		 * by starting up a request and then taking forever to
		 * finish sending it.
		 */

		memcpy(&mask, &ms->svc_fdset, bytes);
		FD_SET (sock, &mask);			/* 4.0 */
		readFds = mask;				/* 4.0 */

		switch (select (FD_SETSIZE, &readFds, (fd_set *)NULL,
			    (fd_set *)NULL, &wait_per_try)) {
		case -1:
			if (errnoGet () == EINTR)
			    continue;
			/* fall through */
