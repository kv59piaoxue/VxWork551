...
--------------------
04a,14jan03,rae  Merged from velocecp branch (SPRs 83258, 83346)
...
STATUS muxAddrResFuncAdd
    ...
    if (ifType < 0 || ifType > MUX_MAX_IFTYPE)
        return (ERROR);
...
*
* This routine takes a string name and a function pointer and finds the
* END devices that has the given name. The function pointer will be called
* as follows:
*   (*pCallbackRtn)(void * pCallbackArg, void * pEnd, int unit);
*
* RETURNS: OK or ERROR if NULL is passed in the variable pDelay
*
*/

STATUS muxIterateByName
    (
    char * pName,               /* device name to search for */
    FUNCPTR pCallbackRtn,	/* pointer to callback routine */
    void * pCallbackArg		/* user-specified callback arg */
    )
    {
    BOOL          found;
    END_TBL_ROW * pEndTblRow;
    ...
    if ((pName == NULL) || (pCallbackRtn == NULL))
        {
        errnoSet (S_muxLib_INVALID_ARGS);
        return ERROR;
        }
    ...
    for (pEndTblRow = (END_TBL_ROW *)lstFirst(&endList);
         ...
         pEndTblRow = (END_TBL_ROW *)lstNext(&pEndTblRow->node))
        {
        /* Compare the device name to the name given. */
        if (STREQ(pEndTblRow->name, pName))
            {
            found = TRUE;
            break;
            }
    ...
    if (found)
        {
        /* Now loop through all of the END devices with pName type. */
        for (pEnd = (END_OBJ *)lstFirst(&pEndTblRow->units); pEnd != NULL;
             pEnd = (END_OBJ *)lstNext(&pEnd->node))
            {
            rc = (*pCallbackRtn) (pCallbackArg, (void *)pEnd,
                                  pEnd->devObject.unit);
            if (rc == ERROR)
                break;
            }
