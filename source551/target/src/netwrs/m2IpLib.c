...
--------------------
01p,14jan03,rae  Merged from velocecp branch (SPRs 72963, 83349, 24213)
...
#include "netinet/vsIp.h" 	/* for _ipCfgFlags definition */
#include "netinet/vsArp.h" 	/* for _llinfo_arp definition */
...
#ifdef ROUTER_STACK
IMPORT STATUS (*pFFLibIpConfigFlagsChangedFn) (int);
#endif /* ROUTER_STACK */
...
STATUS m2IpInit
    ...
    if (vsTbl[myStackNum]->pM2IpGlobals == NULL)
        {
        vsTbl[myStackNum]->pM2IpGlobals =
                              (VS_M2_IP *) KHEAP_ALLOC (sizeof (VS_M2_IP));
        if (vsTbl[myStackNum]->pM2IpGlobals == NULL)
            return (ERROR);

        bzero ( (char *)vsTbl [myStackNum]->pM2IpGlobals, sizeof (VS_M2_IP));
        }
#endif /* VIRTUAL_STACK */
	...
	if (m2RouteTable == NULL)
	    {
#ifdef VIRTUAL_STACK
            KHEAP_FREE (vsTbl[myStackNum]->pM2IpGlobals);
            vsTbl[myStackNum]->pM2IpGlobals = NULL;
#endif /* VIRTUAL_STACK */
...
STATUS m2IpGroupInfoSet
    ...
    if (varToSet & M2_IPFORWARDING)
        ...
        if (pFFLibIpConfigFlagsChangedFn != NULL)
            (*pFFLibIpConfigFlagsChangedFn) (_ipCfgFlags);
#endif /* ROUTER_STACK */
        }
...
STATUS m2IpAtransTblEntryGet
    ...
    maxIndex    = 0x7fffffff;   /* largest possible value */
    ipAddrSaved = -1;		/* Largest IP address */
	...
	/* 
	 * If sdl_alen is 0, the ARP entry is incomplete (no MAC address).
	 * Therefore the entry should be marked as invalid.
	 */

	if (((struct sockaddr_dl *) pRtEntMatch->rt_gateway)->sdl_alen)
            pReqIpAtEntry->ipNetToMediaType =
	   		   (pRtEntMatch->rt_rmx.rmx_expire == 0) ?
                            M2_ipNetToMediaType_static :
                            M2_ipNetToMediaType_dynamic;
        else
	    pReqIpAtEntry->ipNetToMediaType = M2_ipNetToMediaType_invalid;
...
STATUS m2IpRouteTblEntrySet 
...

	{
        ...
        return (rtrequestDelEqui (&route.rt_dst, 
                                  ...
                                  &route.rt_gateway, route.rt_flags, 0,
                                  TRUE, TRUE, NULL)); 
	}
