...
--------------------
02l,14jan03,rae  Merged from velocecp branch (SPR #83249)
...
#define _DHCPC_BASE_BPF_NAME     "/bpf/dhcpc"
...
LOCAL STATUS dhcpcDevDelete (int);
...
/*******************************************************************************
*
* dhcpcDevDelete - Delete all devices created with bpfDevCreate by dhcpcLib.c
*
...
LOCAL STATUS dhcpcDevDelete
    ...
    for (ix = 0; ix < maxLeases; ix++)
        {
        sprintf(devName, "%s/%d", _DHCPC_BASE_BPF_NAME, ix);
...
STATUS dhcpcLibInit 
    ...
    /*
     * Be careful: EVENT_RING_SIZE is defined in 3 differnet files
     * common_subr.h, dhcpcBoot.h and dhcpcCommonLib.h.  Don't ask me why.
     * It's 10 * sizeof(EVENT_DATA), this is not enough for multi-instance
     * dhcpc.
     */
    /* TODO: find a better way to calculate the number needed here. */
    dhcpcEventRing = rngCreate (maxLeases * EVENT_RING_SIZE);
...
void * dhcpcInit
    ...
    sprintf (bpfDevName, "%s/%d", _DHCPC_BASE_BPF_NAME, offset);
