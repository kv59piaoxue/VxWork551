...
--------------------
01z,22jan03,wap  Make init routine backwards compatible
01y,14jan03,rae  Merged from velocecp branch
...
#define _BOOTP_MAX_DEVNAME 	16 	/* /bpf/bootpc/0 to /bpf/bootpc/N */
...
STATUS bootpLibInit
    ...
    {
    return (bootpLibMultiInit (1, maxSize));
    }
...
*
* This routine behaves almost identically to bootpLibInit(), except it
* is designed for systems that need to perform BOOTP requests over several
* interfaces at once. The <numIfs> parameter specifies the maximum number
* of interfaces available in the system. The <maxSize> parameter specifies
* the maximum link level header size, the same as for bootpLibInit().
*
* This routine must be called before using any other library routines.
* It may be called at startup in place of bootpLibInit() if multi-interface
* support is desired, however bootpLibInit() is called by default when
* INCLUDE_BOOTP is defined. The caller may specify BOOTP_MAXSIZE as the
* value for the <maxSize> parameter.
*
* RETURNS: OK, or ERROR if initialization fails.
*
* ERRNO: S_bootpLib_MEM_ERROR;
...
STATUS bootpLibMultiInit
    (
    ulong_t	numIfs,        /* number of interfaces */
    int 	maxSize 	/* largest link-level header, in bytes */
    )
    {
    ...
    for ( ix = 0; ix < numIfs; ix++ )
        {
        sprintf(devName, "/bpf/bootpc/%d", ix);
        if (bpfDevCreate (devName, 1, bufSize) == ERROR)
            {
            for ( --ix; ix > -1; ix-- )
                {
                sprintf(devName, "/bpf/bootpc/%d", ix);
                bpfDevDelete (devName);
                }
            bpfDrvRemove ();
            return (ERROR);
            }
...
STATUS bootpMsgGet
    ...
    char bpfDevName [_BOOTP_MAX_DEVNAME]; 	/* "/bpf/bootpc/0-N" */
    int bpfDev;
    struct ifreq ifr;
    ...
    sprintf (bpfDevName, "/bpf/bootpc/%d", pIf->if_unit);
