...
--------------------
02r,14jan03,rae  Merged from velocecp branch
...
*
* This routine is called during system startup if INCLUDE_ARP is defined. 
...
STATUS arpLibInit (void)
    {
#ifdef VIRTUAL_STACK
    /* Allocate space for former global variables. */

    if (vsTbl[myStackNum]->pArpGlobals == NULL)
        {
        vsTbl[myStackNum]->pArpGlobals = 
                        (VS_ARP *) KHEAP_ALLOC (sizeof (VS_ARP));
        if (vsTbl[myStackNum]->pArpGlobals == NULL)
            return (ERROR);

        bzero ( (char *)vsTbl [myStackNum]->pArpGlobals, sizeof (VS_ARP));
        }
    else
        return (OK);
...
STATUS arpResolve
    ...
    pRt = rtalloc1 ( (struct sockaddr *)&sockInetAddr, 1, 0);
    if (pRt == NULL)
	{
	errno = S_arpLib_INVALID_HOST;
	return (ERROR);
