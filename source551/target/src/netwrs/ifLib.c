...
--------------------
03j,17jan03,vvv  added IFF_DONT_FORWARD (SPR #83295)
03i,14jan03,rae  Merged from velocecp branch (SPR #83252)
...
STATUS ifFlagChange
    ...
    status = ifFlagSet (interfaceName, oldFlags);

    /* Check if interface has just been marked "UP" */

    if ((_ipCfgFlags & IP_DO_ARP_ON_FLAGCHG) && (flags & IFF_UP) &&
        status == OK && on == TRUE)
        {
        /* If so, force the transmission of a gratuitous ARP. */

        pIf = ifunit (interfaceName);
        if (pIf != NULL)
            arpwhohas ((struct arpcom *) pIf, 
                       &(((struct arpcom *)pIf)->ac_ipaddr));
        else
            return (ERROR);
        }
...
* .iP "IFF_DONT_FORWARD (0x2000)"
* Disables forwarding through the interface.
* .LP
*
* NOTE
...
LOCAL int routeNodeDelete
    ...
    void *		pArg       /* pointer to the interface and counter */
    )
    {

    struct ifnet *		ifp =  (struct ifnet *)((int *)pArg)[0];
    int	*			pDeleted = (int *)((int *)pArg)[1];
    struct rtentry *		pRt;
