...
--------------------
03r,14jan03,rae  Merged from velocecp branch
...
#include "netinet/vsNetCore.h" 	/* for inetdomain definition */
#include "arpLib.h"             /* for arpLibInit() prototype */
#endif
...
IMPORT STATUS mbinit (void); 			/* in netinet/uipc_mbuf.c */
...
BOOL            netLibGenInitialized    = FALSE;/* flag */
#endif
...
* specific routines to be executed on a per virtual stack bases.
*
* RETURNS: OK/ERROR
*          
*/
...
STATUS netLibGeneralInit (void)
    ...
    if (netLibGenInitialized)     /* this function was called earlier */
        return (OK);

#ifdef VIRTUAL_STACK
    /*
     * Make sure that the interface list is empty on the first call, since
     * the automatic initialization from the compiler is not available.
     */
    ...
    if (mbinit() != OK)                 /* network buffer initialization */
        return (ERROR);

#ifdef VIRTUAL_STACK
    if (arpLibInit() != OK)
        return (ERROR);			/* initialize the VS part of arpLib */
#endif /* VIRTUAL_STACK */
    ...
    netLibGenInitialized = TRUE;        /* Set the flag */
