...
--------------------
02s,25apr03,rae  merge from wrn_wipro_fix branch (ver 02t) (SPRs 87758, 73874)
02r,14jan03,rae  Merged from velocecp branch (SPR #82643)
...
int ftpHookup
    ...
    /* set SO_LINGER socket option on ctrlSock so that it closes the connection
     * gracefully, when required to close. Fixes SPR 73874. */
    ...
    if (setsockopt (ctrlSock, SOL_SOCKET, SO_LINGER, (char*)&optVal, 
            sizeof (optVal)) == ERROR)  
        {
        FTPLDEBUG ("ftpHookup: error in setsockopt SO_LINGER errno:0x%08x\n", \
            FTPL_DEBUG_ERRORS, errno,1,2,3,4,5);
        close (ctrlSock);
        return (ERROR);
        }
