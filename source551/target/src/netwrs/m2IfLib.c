...
--------------------
02b,20mar03,wap  Move m2If64BitCounters to funcBind (SPR #86776)
02a,27jan03,wap  ifStackTable not traversed correctly during snmpwalk (SPR
                 #83683)
01z,27jan03,vvv  added support for user-defined counters (SPR #83254)
01y,23jan03,vvv  docs: fixed m2PollStatsIfPoll markup
01x,14jan03,rae  Merged from velocecp branch (SPRs 83330, 83342,
                 27993/75392/83256, 79092/83331, 80989)
...
#define M2_POLLINCR_64(flag, counter)					\
    if (pDrvConf->ifValidCounters & (flag))				\
        {                                                               \
        m2IfIncr32Bit (pMib2Data,					\
                       &(pMib2Data->mibIfTbl.if ## counter),		\
                       pDrvCounters->if ## counter & 0xFFFFFFFF);	\
        m2IfIncr64Bit (pMib2Data,					\
                       &(pMib2Data->mibXIfTbl.ifHC ## counter),		\
                       pDrvCounters->if ## counter & 0xFFFFFFFF);	\
        }

#define M2_POLLINCR_64_X(flag, counter)				\
    if (pDrvConf->ifValidCounters & (flag))				\
        {                                                               \
        m2IfIncr32Bit (pMib2Data,					\
                       &(pMib2Data->mibXIfTbl.if ## counter),		\
                       pDrvCounters->if ## counter & 0xFFFFFFFF);	\
        m2IfIncr64Bit (pMib2Data,					\
                       &(pMib2Data->mibXIfTbl.ifHC ## counter),		\
                       pDrvCounters->if ## counter & 0xFFFFFFFF);	\
...
#define M2_POLLINCR_32(flag, srcCounter, dstCounter)			\
    if (pDrvConf->ifValidCounters & (flag))				\
        {                                                               \
        m2IfIncr32Bit (pMib2Data,					\
                       &(pMib2Data->mibIfTbl.if ## dstCounter),		\
                       pDrvCounters->if ## srcCounter & 0xFFFFFFFF);	\
        }
...
int     m2IfCounterHoldAtMax;	/* Allow counters to roll-over or stop at Max */
...
/******************************************************************************
*
* centiSecsGet - get hundredths of a second
*
* The number of hundredths of a second that have passed since this routine was
* first called.
*
* RETURNS: Hundredths of a second since the group was initialized.
*
* SEE ALSO: N/A
*/
LOCAL unsigned long centiSecsGet (void)
    ...
    unsigned long tempTime = tickGet ();

    currCentiSecs = tempTime;
    tempTime *= 100;

    if (startCentiSecs == 0)
	startCentiSecs = tempTime / clkRate;

    if (currCentiSecs != (tempTime / 100))
	currCentiSecs = (currCentiSecs / clkRate) * 100;
    else
        currCentiSecs = tempTime / clkRate;
...
STATUS m2IfInit 
    ...
    if (vsTbl[myStackNum]->pM2IfGlobals == NULL)
        {
        vsTbl[myStackNum]->pM2IfGlobals = 
                               (VS_M2_IF *) KHEAP_ALLOC (sizeof (VS_M2_IF));
        if (vsTbl[myStackNum]->pM2IfGlobals == NULL)
            return (ERROR);

        bzero ( (char *)vsTbl [myStackNum]->pM2IfGlobals, sizeof (VS_M2_IF));

        /*
         * Perform the static initialization which the compiler provides
         * automatically for the non-virtual stack.
         */
    ...
    if (m2InterfaceSem == NULL)
        ...
        if (m2InterfaceSem == NULL)
            {
#ifdef VIRTUAL_STACK
            KHEAP_FREE (vsTbl[myStackNum]->pM2IfGlobals);
            vsTbl[myStackNum]->pM2IfGlobals = NULL;
#endif /* VIRTUAL_STACK */
...
*                     internal counters
*
* This function invokes the EIOCGPOLLSTATS ioctl on a specified
* interface and, if the ioctl succeeds, it updates its associated MIB2
* counters. Only those counters which are marked as valid in the
* interface's ifValidCounters field in the END_IFDRVCONF structure
* will be updated. This routine may be invoked either automatically
* via periodic watchdog timeout, or directly in order to force a
* counter update if the watchdog has not yet expired.
*
* RETURNS: ERROR if reading the counters fails, otherwise OK.
*/
...
STATUS m2PollStatsIfPoll
    ...
    pCookie = pDrvConf->ifMuxCookie;

    /* Get M2_ID from driver. */

    error = muxIoctl (pCookie, EIOCGMIB2233, (caddr_t)&pId);
    ...
    error = muxIoctl (pCookie, EIOCGPOLLSTATS, (caddr_t)&pDrvCounters);
    ...
    M2_POLLINCR_64 (END_IFINUCASTPKTS_VALID, InUcastPkts);
    M2_POLLINCR_64 (END_IFINUCASTPKTS_VALID, OutUcastPkts);
    M2_POLLINCR_64 (END_IFINOCTETS_VALID, InOctets);
    M2_POLLINCR_64 (END_IFOUTOCTETS_VALID, OutOctets);

    M2_POLLINCR_32 (END_IFINERRORS_VALID, InErrors, InErrors);
    M2_POLLINCR_32 (END_IFOUTERRORS_VALID, OutErrors, OutErrors);
    M2_POLLINCR_32 (END_IFINDISCARDS_VALID, InDiscards, InDiscards);
    M2_POLLINCR_32 (END_IFOUTDISCARDS_VALID, OutDiscards, OutDiscards);
    M2_POLLINCR_32 (END_IFINUNKNOWNPROTOS_VALID,
                      InUnknownProtos, InUnknownProtos);

    /*
     * For the multicast and broadcast stats, we need to do the
     * increment on a different table, hence the need for a separate
     * macro. We also need to increment the non-unicast counters.
     */

    M2_POLLINCR_64_X (END_IFINMULTICASTPKTS_VALID, InMulticastPkts);
    M2_POLLINCR_32 (END_IFINMULTICASTPKTS_VALID,
                      InMulticastPkts, InNUcastPkts);

    M2_POLLINCR_64_X (END_IFINBROADCASTPKTS_VALID, InBroadcastPkts);
    M2_POLLINCR_32 (END_IFINBROADCASTPKTS_VALID,
                      InBroadcastPkts, InNUcastPkts);

    M2_POLLINCR_64_X (END_IFOUTMULTICASTPKTS_VALID, OutMulticastPkts);
    M2_POLLINCR_32 (END_IFOUTMULTICASTPKTS_VALID,
                      OutMulticastPkts, OutNUcastPkts);

    M2_POLLINCR_64_X (END_IFOUTBROADCASTPKTS_VALID, OutBroadcastPkts);
    M2_POLLINCR_32 (END_IFOUTBROADCASTPKTS_VALID,
                      OutBroadcastPkts, OutNUcastPkts);
...
*
* This routine installs a pointer <pUsrCounters> in the M2_ID structure 
* specified by <pM2Id>. The installed pointer can be used to add additional 
* counters per interface to support RMON, rfc1643 or any other counters. If a 
* counter already exists the installation fails. The counters can be accessed 
* using the m2UsrCounter field in the M2_ID structure for the interface. The 
* pointer will typically refer to a structure which contains all the counters. 
* For example:
*
* .CS
*     struct counters
...
* .CE
*
* It is the responsibility of the user to allocate and initialize the counter
* structure before calling this routine.
*
* RETURNS: OK, or ERROR
*
...
STATUS m2IfUsrCountersInstall
    ...
    {
    if ((pM2Id == NULL) || (pUsrCounters == NULL))
        {
        errno = S_m2Lib_INVALID_PARAMETER;
        return (ERROR);
        }

    /* Install a new counter only if a counter does not exist. */

    if (pM2Id->m2UsrCounter == NULL)
        pM2Id->m2UsrCounter = pUsrCounters;
    else
        return (ERROR);
...
*
* This routine removes the user-defined counters associated with the interface.
* It is the responsibility of the user to free the counter structure once it
* has been deleted.
* 
* RETURNS: OK, or ERROR if <pM2Id> is NULL
*
* ERRNO: S_m2Lib_INVALID_PARAMETER
*/
