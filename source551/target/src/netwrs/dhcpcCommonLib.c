...
--------------------
02c,11mar03,wap  Correct length calculation in dhcpcOptionAdd() (SPR #86747)
02b,14jan03,rae  Merged from velocecp branch (SPRs 73045, 73593, 83248, 
                 83246)
...
int dhcpcDiscoverRetries = DISCOVER_RETRANS;	/*
                                 		 * Set externally to control
                                                 * how many times client
				                 * will retry DISCOVERs.
						 */
...
UCHAR * dhcpcOptionMerge (struct dhcpcOpts *, UCHAR, UCHAR *, UCHAR *, int);
UCHAR * dhcpcOptCreate (struct dhcpcOpts *, UCHAR, UCHAR *, UCHAR *, int);
...
STATUS dhcpcOptionAdd
    ...
    switch (option)
        ...
        case _DHCP_MTU_PLATEAU_TABLE_TAG: 	/* Option Tag 25 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag25,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
        ...
        case _DHCP_ROUTER_TAG: 			/* Option Tag 3 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag3,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_TIME_SERVER_TAG: 		/* Option Tag 4 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag4,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_NAME_SERVER_TAG: 		/* Option Tag 5 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag5,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_DNS_SERVER_TAG: 		/* Option Tag 6 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag6,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_LOG_SERVER_TAG: 		/* Option Tag 7 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag7,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_COOKIE_SERVER_TAG: 		/* Option Tag 8 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag8,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_LPR_SERVER_TAG: 		/* Option Tag 9 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag9,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_IMPRESS_SERVER_TAG: 		/* Option Tag 10 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag10,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_RLS_SERVER_TAG: 		/* Option Tag 11 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag11,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_NIS_SERVER_TAG: 		/* Option Tag 41 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag41,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_NTP_SERVER_TAG: 		/* Option Tag 42 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag42,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_NBN_SERVER_TAG: 		/* Option Tag 44 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag44,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_NBDD_SERVER_TAG: 		/* Option Tag 45 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag45,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_XFONT_SERVER_TAG: 		/* Option Tag 48 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag48,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_XDISPLAY_MANAGER_TAG: 	/* Option Tag 49 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag49,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_NISP_SERVER_TAG: 		/* Option Tag 65 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag65,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_MOBILEIP_HA_TAG: 		/* Option Tag 68 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag68,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_SMTP_SERVER_TAG: 		/* Option Tag 69 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag69,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_POP3_SERVER_TAG: 		/* Option Tag 70 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag70,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_NNTP_SERVER_TAG: 		/* Option Tag 71 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag71,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_DFLT_WWW_SERVER_TAG: 	/* Option Tag 72 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag72,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_DFLT_FINGER_SERVER_TAG: 	/* Option Tag 73 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag73,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_DFLT_IRC_SERVER_TAG:  	/* Option Tag 74 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag74,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_STREETTALK_SERVER_TAG:  	/* Option Tag 75 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag75,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_STDA_SERVER_TAG: 		/* Option Tag 76 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag76,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_POLICY_FILTER_TAG: 		/* Option Tag 21 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag21,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_STATIC_ROUTE_TAG: 		/* Option Tag 33 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag33,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_VENDOR_SPEC_TAG: 		/* Option Tag 43 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag43,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_REQ_LIST_TAG: 		/* Option Tag 55 */
            pDest = dhcpcOptionMerge (pOptList, option, pOptList->pTag55,
                                      pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
        ...
        case _DHCP_HOSTNAME_TAG: 		/* Option Tag 12 */
            pDest = dhcpcOptCreate (pOptList, option, pOptList->pTag12,
                                    pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
        ...
        case _DHCP_MERIT_DUMP_TAG: 		/* Option Tag 14 */
            pDest = dhcpcOptCreate (pOptList, option, pOptList->pTag14,
                                    pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_DNS_DOMAIN_TAG: 		/* Option Tag 15 */
            pDest = dhcpcOptCreate (pOptList, option, pOptList->pTag15,
                                    pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_ROOT_PATH_TAG: 		/* Option Tag 17 */
            pDest = dhcpcOptCreate (pOptList, option, pOptList->pTag17,
                                    pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_EXTENSIONS_PATH_TAG: 	/* Option Tag 18 */
            pDest = dhcpcOptCreate (pOptList, option, pOptList->pTag18,
                                    pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_NIS_DOMAIN_TAG: 		/* Option Tag 40 */
            pDest = dhcpcOptCreate (pOptList, option, pOptList->pTag40,
                                    pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_NB_SCOPE_TAG: 		/* Option Tag 47 */
            pDest = dhcpcOptCreate (pOptList, option, pOptList->pTag47,
                                    pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_ERRMSG_TAG: 			/* Option Tag 56 */
            pDest = dhcpcOptCreate (pOptList, option, pOptList->pTag56,
                                    pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_CLASS_ID_TAG: 		/* Option Tag 60 */
            pDest = dhcpcOptCreate (pOptList, option, pOptList->pTag60,
                                    pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
        ...
        case _DHCP_CLIENT_ID_TAG: 		/* Option Tag 61 */
            ...
            pDest = dhcpcOptCreate (pOptList, option, pOptList->pTag61,
                                    pData, length);
            if (pDest == NULL)
                {
                free (pReqSpec->clid->id);
            ...
            break;
        case _DHCP_NISP_DOMAIN_TAG: 		/* Option Tag 64 */
            pDest = dhcpcOptCreate (pOptList, option, pOptList->pTag64,
                                    pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_TFTP_SERVERNAME_TAG: 	/* Option Tag 66 */
            pDest = dhcpcOptCreate (pOptList, option, pOptList->pTag66,
                                    pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
            ...
            break;
        case _DHCP_BOOTFILE_TAG: 		/* Option Tag 67 */
            pDest = dhcpcOptCreate (pOptList, option, pOptList->pTag67,
                                    pData, length);
            if (pDest == NULL)
                {
                errno = S_dhcpcLib_OPTION_NOT_STORED;
    ...
    if (option != _DHCP_REQUEST_IPADDR_TAG)
        {
        if (pDest != NULL)
            pReqSpec->pOptList->optlen += *(UINT8 *)(pDest + 1) + 2;
        else
...
* The <pOptList> parameter is a pointer to the option list structure where
* this option will be stored.
*
...
UCHAR * dhcpcOptionMerge
    (
    struct dhcpcOpts *	pOptList,/* pointer to option list */
    UCHAR		option,	/* RFC 2132 tag of desired option */
    ...
    if (pOrig != NULL)
        {
        optlen = * (pOrig + 1);
        /* Subtract length of data that was previously stored here. */
        if (pOptList != NULL)
            pOptList->optlen -= optlen + 2;
        }
    else
        optlen = 0;

    optlen += 2; 	/* Adjust for tag (1 byte) and
			 * length (1 byte) values (2 bytes). */
    ...
    if (pDest != NULL)
        ...
        * (pDest + 1) = newlen - 2;    /* Update length field to new value.
					* (This is the length of the option
					* data _only_, we must not include
					* the option tag and length bytes
					* themselves here.) */
...
* option data for the new (or initial) subfield. The <pOptList> parameter
* is a pointer to the option list structure where this option will be
* stored.
*
* RETURNS: Pointer to new option subfield, or NULL if error occurs.
*
    ...
    if (pDest != NULL)
        ...
        * (pDest + 1) = length;
        ...
        if (pOrig != NULL)
            ...
            /* Subtract length of data that was previously stored here. */
            if (pOptList != NULL)
                pOptList->optlen -= *(UCHAR *)(pOrig + 1) + 2;
            free (pOrig);
            }
        }
