...
--------------------
03l,14jan03,rae  Merged from velocecp branch (SPRs 83262, 29049)
...
IMPORT void ifRouteCleanup(struct ifnet *pIf);

/* globals */

#ifdef ROUTER_STACK
/*
 * Pointer to the Fastpath IPV4 glue library functions. These are set 
 * inside of ffLibInit() when it is called.
 */

STATUS (*pIpv4FFPktSendFn) (M_BLK_ID pMblk, struct ifnet * pIf);
STATUS (*pIpv4FFRouteCompleteFn) (struct rtentry * pRoute, 
			          IP_DRV_CTRL * pDrvCtrl, struct ifnet * pIf,
				  struct sockaddr *);
#endif /* ROUTER_STACK */
...
LOCAL BOOL ipReceiveRtn
    ...
    /* 
     * Check if the packet can be fast forwarded. If not, we'll go through
     * the regular path
     */

    if (pIpv4FFPktSendFn != NULL && type == ETHERTYPE_IP &&
        ((*pIpv4FFPktSendFn) (pMblk, pIfp) == OK))
        {
#ifdef DEBUG
...
#ifdef VIRTUAL_STACK
    /*
     * Set the virtual stack ID so the error will
     * be processed in the correct context.
     */

    virtualStackNumTaskIdSet (pIfp->vsNum);
#endif /* VIRTUAL_STACK */
...
#ifdef ROUTER_STACK
            /* 
             * If we are forwarding the packet, check if we need to
             * supply the forwarding information to Fastpath. We do not
             * want to call Fastpath with a broadcast or multicast address
             */

            if ((m->m_flags & M_FORWARD) && pIpv4FFRouteCompleteFn != NULL && 
                !(m->m_flags & M_MCAST || m->m_flags & M_BCAST))
                (*pIpv4FFRouteCompleteFn) (rt0, pDrvCtrl, ifp, dst);
#endif /* ROUTER_STACK */
