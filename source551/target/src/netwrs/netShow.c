...
--------------------
03y,20mar03,wap  move m2If64BitCounters to funcBind (SPR #86776)
03x,17jan03,vvv  added IFF_DONT_FORWARD (SPR #83295)
03w,14jan03,rae  Merged from velocecp branch (SPRs 83330, 83342)
...
LOCAL BOOL routeEntryPrint
	...
	if (!bufferedRtShow)
	    ...
	    {
	    len = sprintf (routeBuf + bufIndex, "%-16s ", (destAddr->sa_family == AF_INET) ? 
					   aStr : "not AF_INET");
	    bufIndex += len;
	    }
        }
    ...
    if ( (rtType & RT_NET) && (rtType & RT_HST))
		...
		{
		len = sprintf (routeBuf + bufIndex, "%#-10lx ", mask);
		bufIndex += len;
		}
            }
        else
	...
	if (!bufferedRtShow)
	    ...
	    {
	    len = sprintf (routeBuf + bufIndex, "%-4d  %#-4x ", RT_PROTO_GET (destAddr),
	                   TOS_GET (destAddr));
	    bufIndex += len;
	    }
        }
	...
	if (!bufferedRtShow)
	    ...
	    {
	    len = sprintf (routeBuf + bufIndex, "%02x:%02x:%02x:%02x:%02x:%02x ",
			   pChr[0], pChr[1], pChr[2], pChr[3], pChr[4], pChr[5]);
            bufIndex += len;
	    }
	}
	...
	if (!bufferedRtShow)
	    ...
	    {
	    len = sprintf (routeBuf + bufIndex, "%-16s ", 
	...
	{
	len = sprintf (routeBuf + bufIndex, "%#-6hx %-5d  %-10ld %s%d\n", pRt->rt_flags,
