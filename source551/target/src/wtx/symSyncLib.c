...
--------------------
01u,12nov02,dbt  Better handle system symbol table group number (SPR #25509).
...
LOCAL UINT32		kernelGroup = 0;	/* group used for system */
						/* symbol table */
...
void symSyncLibInit ()
    ...
    /*
     * We need to get the group of the module (if any) used to store
     * the network symbol stable. For now, the only way to do this is
     * to verify if there is a module with group "1" (we can consider that 
     * the network symbol table is the first module loaded) and with a ".sym"
     * extension. If we find a module with those characteristics, we also
     * verify it has been loaded with HIDDEN_MODULE flag.
     */

    if ((moduleId = moduleFindByGroup (1)) == NULL)
    	{
	/*
	 * There is no module with group 1, we must be using standalone symbol
	 * table. In this case, use group 0 to store system symbols.
	 */
	 ...
	 kernelGroup = 0;
    	...
    	{
	/*
	 * There is a module with group 1, to make sure it is the module 
	 * created for the network symbol table, verify it has the ".sym"
	 * extension.
	 */

	 if ((moduleName = moduleNameGet (moduleId)) == NULL)
	    {
	    /* We should not enter this case but just to make sure... */
	    ...
	    /*
	     * We use several criterias to detect if the module with group
	     * 1 is the one used for the network symbol table:
	     * - the module name ends with ".sym",
	     * - the module has been loaded with HIDDEN_MODULE flag.
	     */

	    if ((moduleNameLength > 4) &&
	    	(strcmp (moduleName + moduleNameLength - 4, ".sym") == 0) &&
		(moduleId->flags & HIDDEN_MODULE))
		{
		/*
		 * Okay, we can consider that this module is used for
		 * the network symbol table.
		 */
...
STATUS syncSymStart
    ...
    /*
     * initialize symbol table synchroniation library in case it has not
     * already been done.
     */
...
STATUS symSyncTaskInit
    ...
    groupAdd (&symSyncLst, kernelGroup, 1);
