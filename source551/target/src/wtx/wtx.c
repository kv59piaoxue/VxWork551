...
--------------------
07c,27may03,sn   Implemented suggestion from code review by philb
07b,05may03,sn   Removed debug_dmgl tracing
07a,15apr03,sn   Added wtxDemanglerStyleGet, obsoleted wtxDefaultDemanglerStyle
06l,04sep02,dtr  Add support for SPE.
...
#ifdef HOST
    (*phWtx)->demanglerStyle = DMGL_STYLE_INVALID;
#endif
    return (WTX_OK);
    }
...
/*******************************************************************************
*
* wtxDemanglerStyleGet - get target demangling style
*
* This routine returns an appropriate demangling style based on tool used
* to build the target resident C++ runtime. For example if the target
* supports GNU (2.96) C++ the routine returns DMGL_STYLE_GNU.
* If the target is not connected, or the tool is not recognized, the 
* routine defaults to returning the DIAB demangling style (DMGL_STYLE_DIAB).
*
* Typically this routine will be used in conjunction with the demangle
* function:
*
* .CS
*   dmgl = demangle(symbolName, wtxDemanglerStyleGet(hWtx), demangleMode);
* .CE
* 
...
DEMANGLER_STYLE wtxDemanglerStyleGet
    (
    HWTX		hWtx		/* WTX API handle             */
    )
    {
    ...
    if (hWtx->demanglerStyle != DMGL_STYLE_INVALID)
	{
	return hWtx->demanglerStyle;
	}
    memset (&out, 0, sizeof (out));
    memset (&in, 0, sizeof (in));

    in.param.valueType = V_PCHAR;
    in.param.value_u.v_pchar = WTX_DEMANGLER_STYLE_GET_CMD;
    ...
    hWtx->demanglerStyle = (DEMANGLER_STYLE) out.val.value_u.v_uint32;
...
/*******************************************************************************
*
* wtxDefaultDemanglerStyle - see wtxDemanglerStyleGet
*
* Obsolete. Use wtxDemanglerStyleGet instead.
*
* NOMANUAL
*/
...
DEMANGLER_STYLE wtxDefaultDemanglerStyle
    ...
    {
    return wtxDemanglerStyleGet(hWtx);
    }
#endif /* HOST*/
...
/*******************************************************************************
*
* wtxTargetHasSpeGet - check if the target has an spe unit
*
* This routine returns a boolean indicating whether the current target has a
* spe coprocessor available. Errors must be detected using
* wtxErrClear() and wtxErrGet().
*
* RETURNS: TRUE if it has an Spe unit  or FALSE if not or if 
* there was an error.
*/

BOOL32 wtxTargetHasSpeGet 
    (
    HWTX hWtx	/* WTX API handle */
    )
    {
    ...
    return ((hWtx->pTsInfo->tgtInfo.rtInfo.hasCoprocessor & WTX_SPE_MASK) >>
	    WTX_SPE_BITSHIFT);
    }
