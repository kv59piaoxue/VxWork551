...
/* Copyright 1987-2003 Wind River Systems, Inc. */
#include "copyright_wrs.h"

/*
modification history
--------------------
04b,06jan03,bpn  SPR #84653: don't use a rodata variable when intercepting
                 the 'exit' command.
...
LOCAL STATUS execShell
	...
	if (inLine [i] != '#' && inLine [i] != EOS)
		...
		{
		/* 
		 * SPR #84653: we can't execute a statement like:
		 * 	execute ("logout");
		 * as the rodata can be locked out on write by the system, 
		 * and the function get1Redir() (called by execute()) tries to 
		 * write in this string.
		 */

		sprintf (inLine, "logout");
		i = 0;
		}
