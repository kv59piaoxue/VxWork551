...
--------------------
02c,09jun03,mil  Merged from CP1 to e500.
02b,21nov02,pch  SPR 84107: call altivecTaskRegsShow() via pointer
02a,22oct02,hdn  replaced fppSave w _func_fppSaveRtn for PENTIUM.
01z,04sep02,dtr  Adding support for SPE.
...
LOCAL void dbgTlSnglStep
	...
	(* _func_fppSaveRtn) (pTcb->pFpContext);
	fppTaskRegsShow (task);
	(* _func_fppRestoreRtn) (pTcb->pFpContext);
#else
    ...
    if ((pTcb->options & VX_ALTIVEC_TASK) && dbgPrintAltivec
     && (_func_altivecTaskRegsShow != NULL))
	(* _func_altivecTaskRegsShow) (task);
#endif /* _WRS_ALTIVEC_SUPPORT */
#ifdef _WRS_SPE_SUPPORT
    if ((pTcb->options & VX_SPE_TASK) && dbgPrintSpe)
        speTaskRegsShow (task);
#endif /* _WRS_SPE_SUPPORT */
