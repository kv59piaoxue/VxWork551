...
--------------------
02x,08nov02,bpn  SPR#79528: fixed unsigned compare cases.
...
int ledRead
	...
	if (ledId->cmdMode)
		...
		case 'x':
		    strcpy (saveLn, curLn);
		    number = min (number, (int)strlen (curLn) - curPs);
		    ix = 0;
		    do
			{
			ledId->buffer [ix++] = curLn [curPs];
			if (curPs < (int)strlen (curLn) - 1)
		...
		case 'C':
		    ...
		    ix = 0;
                    if (number >= (int)strlen (curLn) - curPs)
                        curLn [curPs] = EOS;
                    else
                        {
                        do
...
LOCAL STATUS deletec
	...
	switch (ch)
		...
		else if (*curPs == (int)strlen (curLn) - 1)
		    *number = 1;		/* get out! */
