...
--------------------
01k,28jan03,wap  fix if_allmulti() (SPR #85939)
...
#include "netinet/vsIp.h" 	/* for _ip_id definition */
	...
	int version;
{
    ...
    if (vsTbl[myStackNum]->pIpMrouteGlobals == NULL)
        {
        vsTbl[myStackNum]->pIpMrouteGlobals = 
                     (VS_IP_MROUTE *) KHEAP_ALLOC (sizeof (VS_IP_MROUTE));
        if (vsTbl[myStackNum]->pIpMrouteGlobals == NULL)
            return (ENOBUFS);

        bzero ( (char *)vsTbl [myStackNum]->pIpMrouteGlobals,
               sizeof (VS_IP_MROUTE));
        }
#endif /* VIRTUAL_STACK */
...
#ifdef VIRTUAL_STACK
    KHEAP_FREE (vsTbl[myStackNum]->pIpMrouteGlobals);
    vsTbl[myStackNum]->pIpMrouteGlobals = NULL;
#endif /* VIRTUAL_STACK */
    ...
    )
{
		...
		if ((M_HASCL(pMbufCopy) || pMbufCopy->m_len < hlen))
