...
--------------------
01j,04feb03,jb  Really resolving SPRs 79632 and 77287
01i,23jan03,jb  Resolving SPRs 79632 and 77287
...
#if (CPU == STRONGARM) || (CPU == XSCALE)
	/* These chips do not support unaligned load, so emulate in software */
	bic	lr, r1, #3		/* lr = r1 & ~3, word aligns pointer */
	ldr	r3, [lr, #0]
	tsts	r1, #2			/* rotate r3 right by 8*(lr&3) */
	movne	r3, r3, ror #16		/* first by 8*(lr&2) */
	tsts	r1, #1			/* then by 8*(lr&1) */
	movne	r3, r3, ror #8		/* Last byte, if required */
#else
        ldr     r3, [r1, #0]            /* Load word */
#endif /* (CPU == STRONGARM) || (CPU == XSCALE) */
