...
LOCAL void *	cacheVr4131Malloc (size_t bytes);
LOCAL STATUS	cacheVr4131Free (void * pBuf);
LOCAL STATUS	cacheVr4131Flush (CACHE_TYPE cache, void *	pVirtAdrs,
			       size_t bytes);
LOCAL STATUS	cacheVr4131Invalidate (CACHE_TYPE cache, void * pVirtAdrs, 
                                    size_t bytes);
LOCAL void * 	cacheVr4131PhysToVirt (void * address);
LOCAL void * 	cacheVr4131VirtToPhys (void * address);
LOCAL STATUS 	cacheVr4131TextUpdate (void * address, size_t bytes);
LOCAL STATUS	cacheVr4131PipeFlush (void);

/* imports */

IMPORT void	sysWbFlush (void);

IMPORT void cacheVr4131DCFlushInvalidate(void * address,
				 size_t byteCount);
IMPORT void cacheVr4131DCFlushInvalidateAll(void);
IMPORT void cacheVr4131ICInvalidate(void * address,
			    size_t byteCount);
IMPORT void cacheVr4131ICInvalidateAll(void);
...
IMPORT void	cacheVr4131Sync (void * vAddr, UINT len);
IMPORT void	cacheVr4131VirtPageFlush (UINT asid, void * vAddr, UINT len);

/* globals */

IMPORT UINT32 cacheVr4131ICacheSize;
IMPORT UINT32 cacheVr4131DCacheSize;
IMPORT UINT32 cacheVr4131SCacheSize;
IMPORT UINT32 cacheVr4131ICacheLineSize;
IMPORT UINT32 cacheVr4131DCacheLineSize;
IMPORT UINT32 cacheVr4131SCacheLineSize;

/**************************************************************************
*
* cacheVr4131LibInit - initialize the R4000 cache library
*
...
STATUS cacheVr4131LibInit
    ...
    {
    cacheVr4131ICacheSize = iCacheSize;
    cacheVr4131DCacheSize = dCacheSize;
    cacheVr4131SCacheSize = sCacheSize;
    cacheVr4131ICacheLineSize = iCacheLineSize;
    cacheVr4131DCacheLineSize = dCacheLineSize;
    cacheVr4131SCacheLineSize = sCacheLineSize;
    ...
    cacheLib.flushRtn = cacheVr4131Flush;		/* cacheFlush() */
    cacheLib.pipeFlushRtn = cacheVr4131PipeFlush;	/* cachePipeFlush() */
    cacheLib.textUpdateRtn = cacheVr4131TextUpdate;/* cacheTextUpdate() */

    cacheLib.invalidateRtn = cacheVr4131Invalidate;    /* cacheInvalidate() */
    cacheLib.clearRtn = cacheVr4131Invalidate;	       /* cacheClear() */

    cacheLib.dmaMallocRtn = (FUNCPTR) cacheVr4131Malloc; /* cacheDmaMalloc() */
    cacheLib.dmaFreeRtn = cacheVr4131Free;               /* cacheDmaFree() */

#ifdef IS_KSEGM
    if (!IS_KSEGM(cacheVr4131LibInit))
	{
	cacheLib.dmaVirtToPhysRtn = (FUNCPTR) cacheVr4131VirtToPhys;
	cacheLib.dmaPhysToVirtRtn = (FUNCPTR) cacheVr4131PhysToVirt;
	_func_mipsCacheSync = (VOIDFUNCPTR) cacheVr4131Sync;
	}
    else
	{
	_func_mipsCacheSync = (VOIDFUNCPTR) KM_TO_K0(cacheVr4131Sync);
	_func_mmuMipsVirtPageFlush = (VOIDFUNCPTR) cacheVr4131VirtPageFlush;
...
/**************************************************************************
*
* cacheVr4131Malloc - allocate a cache-safe buffer, if possible
*
...
LOCAL void * cacheVr4131Malloc
	...
	cacheVr4131DCFlushInvalidate (pBuffer, allocBytes);
...
/**************************************************************************
*
* cacheVr4131Free - free the buffer acquired by cacheMalloc ()
*
...
/**************************************************************************
*
* cacheVr4131Flush - flush all or some entries in a cache
*
...
LOCAL STATUS	cacheVr4131Flush
	...
	case DATA_CACHE:
		...
		cacheVr4131DCFlushInvalidateAll ();
	    else
		cacheVr4131DCFlushInvalidate (pVirtAdrs, bytes);
	    break;
	default:
...
/**************************************************************************
*
* cacheVr4131Invalidate - invalidate all or some entries in a cache
*
...
LOCAL STATUS	cacheVr4131Invalidate
    (
    CACHE_TYPE	cache,			/* Cache to Invalidate */
    void *	pVirtAdrs,		/* Virtual Address */
    size_t	bytes 			/* Number of Bytes to Invalidate */
	...
	case INSTRUCTION_CACHE:
	    if (bytes == ENTIRE_CACHE)
		cacheVr4131ICInvalidateAll ();
	    else
		cacheVr4131ICInvalidate (pVirtAdrs, bytes);
	    break;
	default:
	    errno = S_cacheLib_INVALID_CACHE;
	    return (ERROR);
...
/**************************************************************************
*
* cacheVr4131VirtToPhys - virtual-to-physical address translation
*
* This routine may be attached to the CACHE_DRV structure virtToPhysRtn
* function pointer by cacheVr4131Malloc().  This implies that the virtual
...
LOCAL void * cacheVr4131VirtToPhys
    (
...
/**************************************************************************
*
* cacheVr4131PhysToVirt - physical-to-virtual address translation
*
...
LOCAL void * cacheVr4131PhysToVirt
    (
...
/**************************************************************************
*
* cacheVr4131TextUpdate - invalidate updated text section
*
...
LOCAL STATUS cacheVr4131TextUpdate
    (
    void * address,                     /* Physical address */
    ...
    if (cacheVr4131Flush (DATA_CACHE, address, bytes) != OK)
	return (ERROR);
    return (cacheVr4131Invalidate (INSTRUCTION_CACHE, address, bytes));
    }


/**************************************************************************
*
* cacheVr4131PipeFlush - flush R4000 write buffers to memory
*
