...
--------------------
01l,29oct02,hdn  preserved FP_CONTEXT for FPU exceptions (spr 70252)
01k,22oct02,hdn  added FPU exception (16) support (spr 70252)
...
void excExcHandle
    ...
    {
    EXC_INFO     excInfo;
    INT8         fpBuf[FPX_STATE_FRAME_SIZE + 32];
    FP_CONTEXT * pFpContext = (FP_CONTEXT *)((int)(fpBuf + 32) & ~0x1f);
    FP_CONTEXT * pFpContextExc;
    X86_EXT *    pExt       = (X86_EXT *)taskIdCurrent->reserved2;
    ...
    /* save FP_CONTEXT if it is "FPU floating-point exception" (16) */

    if (vecNum == IN_CP_ERROR)
	{
	excInfo.reserved0 = (UINT32)pFpContext;
	excInfo.valid |= EXC_FP_CONTEXT;
	(* _func_fppSaveRtn) (pFpContext);
	}
    ...
    /* restore the saved FP_CONTEXT. allow hook rtns to use the FPU */

    if (vecNum == IN_CP_ERROR)
	{
	(* _func_fppRestoreRtn) (pFpContext);

        /* update the pTcb->pFpContext for fppTaskRegs[GS]et() */

        if (taskIdCurrent->pFpContext != NULL)
            bcopy ((char *) pFpContext, (char *) taskIdCurrent->pFpContext,
	           sizeof (FP_CONTEXT));		/* copy FP_CONTEXT */

	/* remember the FP_CONTEXT that caused the FPU exception */

	taskIdCurrent->excInfo.reserved0 = 0;
	taskIdCurrent->excInfo.valid &= ~EXC_FP_CONTEXT;
        if (pExt != NULL)
	    {
            pFpContextExc = (FP_CONTEXT *)pExt->reserved1;
	    if (pFpContextExc != NULL)
		{
		taskIdCurrent->excInfo.reserved0 = (UINT32)pFpContextExc;
		taskIdCurrent->excInfo.valid |= EXC_FP_CONTEXT;
                bcopy ((char *) pFpContext, (char *) pFpContextExc,
	               sizeof (FP_CONTEXT));		/* copy FP_CONTEXT */
		}
	    }
	}
    ...
    /* just in case the swap hook routine did not restore it */

    if (vecNum == IN_CP_ERROR)
	(* _func_fppRestoreRtn) (taskIdCurrent->pFpContext);
