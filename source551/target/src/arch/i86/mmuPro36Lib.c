...
--------------------
01j,19nov02,max  Added DIAB inline assembling
...
#ifdef _DIAB_TOOL
asm void diab_int_lock_36(unsigned int Mask)
{
% reg Mask
	pushf
	popl	Mask
	andl	$0x00000200, Mask
	cli
% mem Mask
	pushf
	popl	Mask
...
asm void diab_int_unlock_36(unsigned int Mask)
{
% reg Mask; lab L
	testl	$0x00000200, Mask
	jz	L
	sti
L:
% mem Mask; lab L
...
#undef INT_LOCK
#define INT_LOCK(oldLevel)	diab_int_lock_36(oldLevel)
#undef INT_UNLOCK
#define INT_UNLOCK(oldLevel)	diab_int_unlock_36(oldLevel)
...
#define MMU_ON() \
    do { int tempReg; _WRS_ASM ("movl %%cr0,%0; orl $0x80010000,%0; \
    movl %0,%%cr0; jmp 0f; 0:" : "=r" (tempReg) : : "memory"); } while (0)

#define MMU_OFF() \
    do { int tempReg; _WRS_ASM ("movl %%cr0,%0; andl $0x7ffeffff,%0; \
    movl %0,%%cr0; jmp 0f; 0:" : "=r" (tempReg) : : "memory"); } while (0)

#define WP_ON() \
    do { int tempReg; _WRS_ASM ("movl %%cr0,%0; orl $0x00010000,%0; \
    movl %0,%%cr0; jmp 0f; 0:" : "=r" (tempReg) : : "memory"); } while (0)

#define WP_OFF() \
    do { int tempReg; _WRS_ASM ("movl %%cr0,%0; andl $0xfffeffff,%0; \
    movl %0,%%cr0; jmp 0f; 0:" : "=r" (tempReg) : : "memory"); } while (0)

/* inline version of mmuI86TLBFlush() */

#define	MMU_TLB_FLUSH() \
    do { int tempReg; _WRS_ASM ("pushfl; cli; movl %%cr3,%0; \
...
LOCAL void mmuCurrentSet 
	...
	/* perform the deferred TLB and Cache flush */
        ...
        if (sysProcessor != X86CPU_386)
            _WRS_ASM ("wbinvd");	/* flush the entire cache */
